...
- 新增 GitHub Actions CI 门禁（windows-latest）：pytest+ruff、eslint+tsc+vitest、cargo test 随每次提交运行。
- 修复回测降级链路：桌面端直连回测增加 300 秒超时与进程回收，回测卡死不再导致界面永久等待。
- 错误提示结构化：本地服务错误响应携带稳定错误码，前端按错误码展示中文提示，报错不再因后端文案变动而失效。
- 数据源设施收敛：符号规范化、数值解析、UA 与重试策略统一为单一实现；资金流新浪备用源的 9xx 代码段与实时行情保持一致（沪市前缀）。
- 静默失败可见化：15 处异常路径补齐日志；本地服务不再跨模块访问私有内部。
- 仓库清理：移除过时的架构审计文档与一次性发布计划文档。
