...
- `turnover_rate`：换手率。
- `pre_close`：前收盘价。
- `float_market_cap`：流通市值，用于市值区间策略。
- `total_market_cap`：总市值，用于扩展筛选和展示。
- `source`：数据来源。
- `updated_at`：更新时间。
...
`market_cap` 是第一阶段必须完善的数据能力。默认策略已经包含流通市值区间，如果市值长期为空，回测结果就不可信。市值优先通过历史股本数据和日线收盘价计算：

- `float_market_cap = close * listed_a_shares`
- `total_market_cap = close * total_shares`
- 股本变更按 `change_date` 向后生效。
- 当历史股本缺失时，记录缺口并尝试备用源补齐。
...
日线、股票池、交易日历和市值必须足够支撑基础全市场回测和默认策略。资金流、行业、ST、停牌等增强数据可以提升策略筛选能力，但缺失这些增强数据不能阻塞不依赖它们的策略。
...
- `fetch_share_history(symbol)`：获取历史股本，用于计算市值。
...
6. 拉取这只股票的历史股本数据。
7. 将股本数据按交易日向后填充，计算 `float_market_cap` 和 `total_market_cap`。
8. 归一化数据，立即写入对应年份的 Parquet 分区。
9. 每完成一只股票，就更新 `symbol_sync_state`。
10. 单只股票失败时记录错误并继续处理下一只，不中断整个全市场任务。
...
- 检查默认市值字段 `float_market_cap` 是否覆盖策略所需日期。
- 检查策略需要的字段是否存在。
...
- 支持启用/禁用已有条件。
- 支持编辑默认策略中已有条件的参数。
- 当某个条件依赖当前缺失的数据时，界面必须提示。市值条件缺失应作为默认策略的严重预检问题；资金流等可选条件缺失时提示用户禁用对应条件或补充数据。
...
- `adata` 历史股本失败时，记录市值缺口；如果备用源能提供流通市值，则用备用源补齐。
...
第一阶段的关键里程碑是：能从 2015-01-01 起把全市场日线和市值字段成功导入本地仓库，即使资金流等其他增强数据还不完整，也能运行基础本地回测和默认市值策略。
